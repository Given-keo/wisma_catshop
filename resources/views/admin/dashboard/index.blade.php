            ...
            <div class="card-body p-4 p-md-5 position-relative">
                <div class="row align-items-center relative z-1">
                    <div class="col-md-8">
                        ...
                        <h2 class="display-6 fw-bold mb-3 text-white">Selamat Datang, {{ auth()->user()->name }}! 👋</h2>
                        <p class="mb-0 fs-6" style="opacity: 0.9; max-width: 550px;">
                            Pantau aktivitas pelanggan, kelola transaksi, dan pastikan seluruh layanan Wisma CatShop berjalan lancar hari ini.
                        </p>
                    </div>
                </div>
                <div class="position-absolute d-none d-md-block" style="right: 5%; top: 50%; transform: translateY(-50%); opacity: 0.15; color: #ffffff;">
                    <i class="fas fa-store" style="font-size: 8rem;"></i>
                </div>
            </div>
        </div>
    </div>
...
<div class="row mb-4 g-4">
        ...
        <div class="card border-0 shadow-sm h-100 hover-elevate" style="border-radius: 12px;">
            <div class="card-body p-4 d-flex align-items-center">
                ...
                <div>
                    ...
                    <h3 class="mb-0 fw-bold text-dark">{{ $totalPelanggan ?? 0 }}</h3>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card border-0 shadow-sm h-100 hover-elevate" style="border-radius: 12px;">
            <div class="card-body p-4 d-flex align-items-center">
                ...
                <div>
                    ...
                    <h3 class="mb-0 fw-bold text-dark">{{ $menungguKonfirmasi ?? 0 }}</h3>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card border-0 shadow-sm h-100 hover-elevate" style="border-radius: 12px;">
            <div class="card-body p-4 d-flex align-items-center">
                <div class="bg-light text-info rounded-circle d-flex align-items-center justify-content-center me-3 shadow-sm" style="width: 60px; height: 60px; background-color: rgba(23, 162, 184, 0.1) !important;">
                    <i class="fas fa-cat fs-3"></i>
                </div>
                <div>
                    <p class="text-muted mb-1 small text-uppercase fw-bold">Sedang Boarding</p>
                    <h3 class="mb-0 fw-bold text-dark">{{ $kucingDititipkan ?? 0 }}</h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-3 col-sm-6">
        <div class="card border-0 shadow-sm h-100 hover-elevate" style="border-radius: 12px;">
            <div class="card-body p-4 d-flex align-items-center">
                <div class="bg-light text-success rounded-circle d-flex align-items-center justify-content-center me-3 shadow-sm" style="width: 60px; height: 60px; background-color: rgba(40, 167, 69, 0.1) !important;">
                    <i class="fas fa-wallet fs-3"></i>
                </div>
                <div>
                    <p class="text-muted mb-1 small text-uppercase fw-bold">Omset Bulan Ini</p>
                    <h4 class="mb-0 fw-bold text-success">Rp {{ number_format($pendapatanBulanIni ?? 0, 0, ',', '.') }}</h4>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-12">
        <div class="card shadow-sm border-0" style="border-radius: 15px; overflow: hidden;">
            <div class="card-header bg-white d-flex justify-content-between align-items-center py-3 border-0">
                <h5 class="mb-0 fw-bold text-gray-800"><i class="fas fa-history text-brand me-2"></i>Transaksi Terbaru</h5>
                <a href="{{ route('admin.transaksi.bookings.index') }}" class="btn btn-sm btn-outline-brand rounded-pill px-3">Lihat Semua Data</a>
            </div>
                        ...
                        <tbody class="border-top">
                            @forelse($transaksiTerbaru ?? [] as $trx)
                                <tr class="border-bottom">
                                    ...
                                    <td>
                                        <a href="{{ route('admin.transaksi.bookings.show', $trx->id) }}" class="fw-bold text-decoration-none text-brand">
                                            {{ $trx->booking_code }}
                                        </a>
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            ...
                                            <span class="fw-bold text-gray-800">{{ $trx->user->name ?? '-' }}</span>
                                        </div>
                                    </td>
                                    <td>
                                ...
                                <tr>
                                    <td colspan="6" class="text-center py-5">
                                        <div class="d-flex flex-column align-items-center justify-content-center">
                                            ...
                                            <p class="text-muted small mb-0">Belum ada transaksi terbaru dari pelanggan yang masuk ke sistem.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
